package com.uni.cloud.lang.misc;

public class Option {

    public static final String host = "www.gubstech.com";   //"18.191.92.76";
    public static final int port = 61051;
    public static final int ssl_port = 61052;
    public static final String test_host =  "139.224.221.196";
    public static final String  dns_name =  "gubstech.com";

    public static final int TTS_FEMALE = 2;
    public static final int TTS_MP3_AUDIO_CODE = 2;

    public static final boolean Log_IsOpen = true;

    public static final boolean FlyTek_IsOpen = true;

    public static final boolean Ssl_IsOpen = true;    //false;   // true;

    public static final boolean test_IsOpen = true;

    public static final boolean FullScreen_IsOpen = false;

    public static final boolean Heart_IsOpen = true;

    public static final int heart_time= 5*60;      //s

    public static final int heart_fail_max_num= 3;      //s

    public static final boolean IsK8s_test = true;  //false
    public static final int k8s_test_port = 32001;      //6001;
    public static final String k8s_test_host = "www.gubstech.com";               //"18.217.235.82";   //"139.224.221.196";           // "18.191.92.76";   // "139.224.221.196";


}
